...
func TestNewTreeSetFromSlice(t *testing.T) {
	...
	var set BaseSet[wrapper.Int] = NewTreeSetFromSlice[wrapper.Int]([]wrapper.Int{1, 4, -5, 2, 1})
...
func TestEqualTreeSet(t *testing.T) {
	...
	var set Set[wrapper.Int] = NewTreeSet[wrapper.Int](1, 2, 3, 5)
	var setTest Set[test] = NewTreeSet[test](test{n1: 1, n2: 2}, test{n1: -2, n2: -4})
	...
	if !set.Equal(NewHashSetFromSlice([]wrapper.Int{2, 1, 3, 5})) {
	...
	if !setTest.Equal(NewTreeSet[test](test{n1: 2, n2: 1}, test{n1: 0, n2: -6})) {

		t.Log("sets are not equals")
		t.Fail()

	}
	if setTest.Equal(NewHashSet[test](test{n1: 1, n2: 1}, test{n1: -2, n2: -4})) {
